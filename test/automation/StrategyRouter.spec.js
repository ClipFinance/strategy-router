...
describe("StrategyRouter upkeep automation", function () {
  ...
  const itBehavesLikeInitialState = () => {
    it("Countdown not started", async () => {
      const currentCycleFirstDepositAt = ethers.BigNumber.from(
        await provider.getStorageAt(router.address, 101)
      );
      expect(currentCycleFirstDepositAt).to.be.equal(0);
    });

    it("Deposits count is zero", async () => {
      const currentCycleDepositsCount = ethers.BigNumber.from(
        await provider.getStorageAt(router.address, 104)
      );
    ...
    describe("Current cycle is active", function () {
      ...
      describe("First deposit withdrawn", function () {
        beforeEach(async function () {
          ...
          let receipts = await receiptContract.getTokensOfOwner(
            depositor1.address
          );
          await router
            .connect(depositor1)
            .withdrawFromBatch([receipts[receipts.length - 1]]);
        });
  ...
  describe("Test automation of allocation to strategies", function () {
    beforeEach(async () => {
      ...
      await admin.setAllocationWindowTime(cycleAllocationWindowTime);
    });
    ...
    describe("First deposit to batch by user #1 asset A", function () {
      beforeEach(async function () {
        ...
        await assetA
          .connect(depositor1)
          .approve(router.address, parseUsdc("1500"));
        await router
          .connect(depositor1)
          .depositToBatch(assetA.address, parseUsdc("1500"), "");
      });
      ...
      describe("Second deposit", function () {
        describe("From user #1 in asset A", function () {
          beforeEach(async function () {
            ...
            await assetA
              .connect(depositor2)
              .approve(router.address, parseUsdc("5000"));
            await router
              .connect(depositor2)
              .depositToBatch(assetA.address, parseUsdc("5000"), "");
          });
        ...
        describe("From user #1 in asset B", function () {
          beforeEach(async function () {
            ...
            await assetB
              .connect(depositor2)
              .approve(router.address, parseUsdt("5000"));
            await router
              .connect(depositor2)
              .depositToBatch(assetB.address, parseUsdt("5000"), "");
          });
        ...
        describe("From user #2 in asset A", function () {
          beforeEach(async function () {
            ...
            await router
              .connect(depositor2)
              .depositToBatch(assetB.address, parseUsdc("5000"), "");
          });
  ...
  describe("AllocationWindowTime affects upkeep", function () {
    ...
    it("default allocation window time is minimum", async () => {
      const allocationWindowTime = ethers.BigNumber.from(
