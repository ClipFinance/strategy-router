...
async function main() {
  ...
  // ~~~~~~~~~~~ DEPLOY StrategyRouter, RouterAdmin, Batch,BatchOut, SharesToken, ReceiptNFT contracts ~~~~~~~~~~~
  console.log(" ");
  console.log("Deploying StrategyRouter...");

  const ROUTER_LIB = "0xC7d256C5E9898C07820466de78Ebe46254938bf0"; // we can deploy new lib and use it here
  ...
  console.log("Deployed StrategyRouter address:", router.address);
  ...
  console.log("Deployed Batch address:", batch.address);
  ...
  console.log("Deployed BatchOut address:", batchOut.address);
  ...
  console.log("Deployed SharesToken address:", sharesToken.address);
  ...
  console.log("Deployed ReceiptNFT address:", receiptContract.address);
  ...
  await dodoUsdt.deployed();
  console.log("Deployed Dodo Usdt Strategy address:", dodoUsdt.address);
  ...
  const stargateUsdt = await upgrades.deployProxy(
    StrategyFactory,
    [
      owner.address,
      parseUsdt((1_000_000).toString()), // TODO change to real value on production deploy
  ...
  await stargateUsdt.deployed();
  console.log("Deployed Stargate Usdt Strategy address:", stargateUsdt.address);
  ...
  const oracleTokens = [hre.networkVariables.wbnb];
  ...
  const hayIdleStrategy = await deployProxyIdleStrategy(
    owner,
    batch,
    router,
    admin.address,
